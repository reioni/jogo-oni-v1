//Defina modelos uma vez.A origem é o centro do modelo.

// Um cubo simples, 8 vértices, 6 quads.
// O padrão é um comprimento de borda de 2 unidades, pode ser influenciado com o `escala`.
...
// Não é muito otimizado - muitos vértices duplicados são gerados.
function makeRecursiveCubeModel({ recursionLevel, splitFn, color, scale=1 }) {
	const getScaleAtLevel = level => 1 / (3 ** level);

	// Podemos modelar o nível 0 manualmente.É apenas um único cubo centrado.
	let cubeOrigins = [{ x: 0, y: 0, z: 0 }];

	// Substitua recursivamente cubos por cubos menores.
	...
	// Gere um modelo de cubo único e escala -o.
	const cubeModel = makeCubeModel({ scale: 1 });
	cubeModel.vertices.forEach(scaleVector(getScaleAtLevel(recursionLevel)));

	// Calcule os valores de origem máxima x, y ou z serão.
// O mesmo resultado que `math.max (... cubeorigins.map (o => o.x))`, mas muito mais rápido.
	const maxComponent = getScaleAtLevel(recursionLevel) * (3 ** recursionLevel - 1);

	// Coloque a geometria do cubo em cada origem.
	cubeOrigins.forEach((origin, cubeIndex) => {
		// Para calcular a oclusão (sombreamento), encontre o componente de origem com o maior
// magnitude e normalize -o em relação ao `maxcomponent`.
		const occlusion = Math.max(
			Math.abs(origin.x),
			Math.abs(origin.y),
			Math.abs(origin.z)
		) / maxComponent;
		// Em iterações mais baixas, a oclusão parece melhor iluminada um pouco.
		const occlusionLighter = recursionLevel > 2
			? occlusion
			: (occlusion + 0.8) / 1.8;
			// clone, traduz vértices para origem e aplicar escala
		...
		// Clone polys, Shift referenciado em índices de vértice e calcule a cor.
...
// O: Vector3D - Posição da origem do Cube (centro).
// s: vetor3d - determina o tamanho da esponja de menger.
...
// ajudante para otimizar modelos, mesclando vértices duplicados dentro de um limite,
// e removendo todos os polys que compartilham os mesmos vértices.
// MUTATE diretamente o modelo.
function optimizeModel(model, threshold=0.0001) {
	...
	// Premissas:
// 1. Cada poli não terá duplicatas ou 1 duplicado.
// 2. Se dois polys forem iguais, ambos estão ocultos (dois cubos tocando),
